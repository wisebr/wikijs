  ...
  v-app(:dark='$vuetify.theme.dark').tags
    ...
    v-content.grey(:class='$vuetify.theme.dark ? `darken-4-d5` : `lighten-3`')
      ...
      .text-center.pt-10(v-if='selection.length < 1')
        img(src='/docs/_assets/svg/icon-price-tag.svg')
      ...
      .px-5.py-2(v-else)
        v-data-iterator(
          ...
          template(v-slot:no-data)
            .text-center.pt-10
              img(src='/docs/_assets/svg/icon-info.svg')
...
export default {
  ...
  methods: {
    ...
    goTo (page) {
      window.location.assign(`/docs/${page.locale}/${page.path}`)
    }
  },
  apollo: {
    tags: {
