  ...
  .search-results(v-if='searchIsFocused || (search && search.length > 1)')
    .search-results-container
      .search-results-help(v-if='!search || (search && search.length < 2)')
        img(src='/docs/_assets/svg/icon-search-alt.svg')
      ...
      .search-results-none(v-else-if='!searchIsLoading && (!results || results.length < 1)')
        img(src='/docs/_assets/svg/icon-no-results.svg', alt='No Results')
          ...
          template(v-for='(item, idx) of results')
            v-list-item(@click='goToPage(item)', @click.middle="goToPageInNewTab(item)", :key='item.id', :class='idx === cursor ? `highlighted` : ``')
              v-list-item-avatar(tile)
                img(src='/docs/_assets/svg/icon-selective-highlighting.svg')
              v-list-item-content
...
export default {
  ...
  methods: {
    ...
    goToPage(item) {
      window.location.assign(`/docs/${item.locale}/${item.path}`)
    },
    goToPageInNewTab(item) {
      window.open(`/docs/${item.locale}/${item.path}`, '_blank')
    }
  },
  apollo: {
    response: {
