...
export default {
  ...
  methods: {
    ...
    viewSource (versionId) {
      window.location.assign(`/docs/s/${this.locale}/${this.path}?v=${versionId}`)
    },
    download (versionId) {
      window.location.assign(`/docs/d/${this.locale}/${this.path}?v=${versionId}`)
    },
    ...
    branchOffHandle ({ locale, path }) {
      window.location.assign(`/docs/e/${locale}/${path}?from=${this.pageId},${this.branchOffOpts.versionId}`)
    },
    toggleViewMode () {
      this.viewMode = (this.viewMode === 'line-by-line') ? 'side-by-side' : 'line-by-line'
    },
    goLive () {
      window.location.assign(`/docs/${this.path}`)
    },
